...
1. Package unsigned installers in parallel:
   - macOS arm64 `.dmg` on `macos-15` via `dist:mac`
   - macOS x64 `.dmg` on `macos-15-intel` via `dist:mac:x64` (`node-pty` compiles on a real Intel machine). `macos-15-intel` is GitHub’s last x86_64 macOS image and is removed in August 2027.
   - Windows x64 portable `.exe` + `.msi` on `windows-latest` via `dist:win`
   - Linux x64 `.AppImage` + `.deb` on `ubuntu-latest` via `dist:linux`

   `CSC_IDENTITY_AUTO_DISCOVERY=false` so electron-builder does not look for a signing identity. `--publish never` so it does not try to create a GitHub Release itself (CI would otherwise demand `GH_TOKEN` after the installers are already built).

2. Create (or update) one GitHub Release with the installers and notes taken from the top `CHANGELOG.md` section, plus the macOS `xattr` instruction and the Linux install note.

Artifact names are `${name}-${version}-${os}-${arch}.${ext}`, for example `git-mastery-0.0.1-mac-arm64.dmg`, `git-mastery-0.0.1-mac-x64.dmg`, `git-mastery-0.0.1-linux-x64.AppImage`, and `git-mastery-0.0.1-linux-x64.deb`. `latest*.yml` and `.blockmap` files are not attached; there is no in-app updater.
...
There is no Apple or Windows signing certificate in CI, and this repo does not require secrets. Both Mac DMGs are unsigned. macOS users still need:

```bash
xattr -rc /Applications/git-mastery.app
```

The Linux AppImage and deb are unsigned the same way. Linux has no quarantine step.

## Rejected alternatives

- **Release on every merge to `main`.** Builds compile `node-pty` across four package jobs and take several minutes. Shipping should be an explicit action.
...
- **`@semantic-release/github` creating the release in the version job.** The GitHub Release would be public with no binaries for the whole packaging window, and a failed build would leave it empty. Creating the release after the installers land avoids that.
...
- **Cross-compile the Intel DMG on the arm64 `macos-15` job.** One fewer Mac runner, but `node-pty` is native. A missed x64 rebuild ships an arm64 addon inside the Intel app. Native `macos-15-intel` until that image is retired.
- **Linux arm64 desktop.** The CLI asset `gitmastery-<version>-linux-arm64` exists. This cut is x64 AppImage and deb only.
