 ...
 * turn is a neutral filled bubble, the assistant's is unboxed body copy so long
...
export const MessageContent = ({
  ...
  <div
    className={cn(
      "flex min-w-0 flex-col gap-2 text-sm leading-[1.6] text-fg",
      "group-[.is-user]:max-w-[85%] group-[.is-user]:rounded-3xl group-[.is-user]:bg-subtle group-[.is-user]:px-4 group-[.is-user]:py-2.5",
  ...
  ({ className, ...props }: MessageResponseProps) => (
    <Streamdown
      className={cn(
        ...
        "[&_ol]:my-2 [&_ol]:list-decimal [&_ol]:pl-5 [&_ul]:my-2 [&_ul]:list-disc [&_ul]:pl-5 [&_li]:my-1",
        ...
        "[&_:not(pre)>code]:rounded [&_:not(pre)>code]:border [&_:not(pre)>code]:border-border [&_:not(pre)>code]:bg-subtle [&_:not(pre)>code]:px-1 [&_:not(pre)>code]:py-0.5 [&_:not(pre)>code]:font-mono [&_:not(pre)>code]:text-[12.5px] [&_:not(pre)>code]:text-fg",
